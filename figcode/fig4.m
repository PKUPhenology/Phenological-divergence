clear
clc
load('K:\workspace\result\animal_clim_carryover_effect_dailytp_interphase.mat');
sta=cell2mat(stace(2:end,:));
phlist={'Spring departure','First occurrence','Peak occurrence','Nesting','Foraging','Breeding','Raising offspring','Molting','Autumn arrive','Last occurrence'}';
[spc,spli]=xlsread('K:\workspace\data\animal_phen_class.xlsx','Sheet1','A2:D951');
sta(:,49)=sta(:,4);
sta(:,4)=[];
nsta=sta;
...
% %at community level
spc_obj=3;%1 aves, 2 mammalia, 3 amphibia, 6 insecta
lnn=[1 3 4 5 8 9];%mammalia amphibia [1 3 5 8 9];%Aves  [1 4 6 7 8 9];%insecta %
sta=nsta(nsta(:,48)==spc_obj,:);%%%animal
...
ch1=h1.Children;
set(ch1,'color','#0072BD','Linestyle','-','markersize',1)
