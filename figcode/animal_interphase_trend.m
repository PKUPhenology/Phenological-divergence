load('K:\workspace\result\animal_interphase_trend.mat')
for i=1:length(spc)
sta(sta(:,2)==spc(i,1),8)=spc(i,4);
end
sta=sta(sta(:,8)==spc_obj,:);
DATA_all=nan(nrc,nrc);
DATA_allsd=nan(nrc,nrc);
DATA_allr=nan(nrc,nrc);
for i=1:nrc-1
for j=i+1:nrc
tran=sta(sta(:,3)==i&sta(:,4)==j,5);
num(i,j)=length(tran);
if length(tran)>1
...
ylabel(ch,'Percentage of shrinked interphases (%)','fontname','Arial','fontsize',8);
...
text(m(sm,2),m(sm,1)+0.175,sprintf('%s',strcat(['¡À ',num2str(round(bci(m(sm,1),m(sm,2))*100)/10)])),'FontSize',8,'fontname','Arial','color','k','horizontalalignment','center')
hold on
if rp(m(sm,1),m(sm,2))<20
...
set(gca,'xtick',1:1:nn,'xticklabel',phlist,'fontname','Arial','fontsize',8,'XTickLabelRotation',45)
set(gca,'ytick',1:1:nn,'yticklabel',flipud(phlist),'fontname','Arial','fontsize',8)
